                    ...
                    try:
                        ...
                        if len(response) == 1:
                            ...
                            for sentence in sentences:
                                orginalSentence = sentence
                                sentence = sentence[0]

                                #[['有机ashwagandha粉末', [5]], ['有机南非醉茄粉末', [1]]]
                                #[['你叫什么名字', [4, 5, 1]]]
                                #替补组合
                                alternateGrop = orginalSentence[len(orginalSentence)-1]
                                #第一结果含有英文且还有替补组合
                                if lang_tgt == 'zh-CN'  and bool(re.search('[a-z]', sentence.lower())) and isinstance(alternateGrop,list):
                                    for group in alternateGrop:
                                        newSentence = group[0]
                                        #提取没有英文或者含有中文的
                                        if bool(re.search('[a-z]', newSentence.lower())) == False or helper.contain_chinese(newSentence):
